...
CONSOLE_VERBOSE = os.getenv("RAM_LAUNCH_CONSOLE_VERBOSE", "1").strip().lower() not in {
    "0",
    "false",
    "no",
    "off",
...
class TraceLogger:
    ...
    def write(self, message: str = "") -> None:
        ...
        with self.path.open("a", encoding="utf-8") as stream:
            stream.write(line + "\n")
        if CONSOLE_VERBOSE or message.startswith(
            (
                "Log file:",
                "Config:",
                "STEP ",
            ...
            if preflight.captcha_detected:
                ...
                for captcha_attempt in range(1, CAPTCHA_MAX_ATTEMPTS + 1):
                    ...
                    if completion is not None:
                        ...
                        elif completion.challenge_id == preflight.challenge_id:
                            replay_accepted = replay_completed_gamejoin(
                                session, preflight, completion
                            )
                    else:
                        LOG.write(
                    ...
                    if replay_accepted:
                        LOG.write(
                            ...
                            f"attempt={captcha_attempt} result=ACCEPTED"
                        )
                        break
                    LOG.write(
                        "CAPTCHA_ATTEMPT_RESULT "
